...
test('desktop shell has named controls and no horizontal overflow', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: '创建整理流程' })).toBeVisible()
  await expect(page.getByRole('heading', { name: '选择要处理的文件夹' })).toBeVisible()
  await expect(page.getByRole('button', { name: '保存并生成预览' })).toBeVisible()
  await expect(page.getByText('options (JSON)')).toHaveCount(0)
})
